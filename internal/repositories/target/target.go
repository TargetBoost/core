...
func (r *Repository) GetTargetsToAdmin() []models.TargetToAdmin {
	...
	r.db.Table("targets t").Select("t.id, t.uid, t.title, t.link, t.icon, t.status, t.count, t.total, t.cost, t.total_price, u.login").Joins("inner join users u on t.uid = u.id").Find(&t)
